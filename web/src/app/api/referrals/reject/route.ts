...
import { buildRejectReferralXdr, submitSignedTransaction } from "@/lib/stellar";
  ...
  try {
    ...
    if (referral.status !== "PENDING") {
      return NextResponse.json(
        { error: `Referral is already ${referral.status.toLowerCase()}, cannot reject.` },
        { status: 409 }
      );
    }

    if (step === "build") {
      const xdr = await buildRejectReferralXdr({ businessPublicKey, onChainId });
    ...
    console.error("POST /api/referrals/reject failed:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Unknown error rejecting referral." },
      { status: 500 }
    );
  }
}
